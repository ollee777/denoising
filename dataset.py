...
def Im2Patch(img, win, stride=1):
    ...
    Y = np.zeros([endc, win*win,TotalPatNum], np.float16)
    for i in range(win):
        for j in range(win):
...
def prepare_data(root, data_path, patch_size, stride, aug_times=1):
    ...
    h5f = h5py.File('data/train16.h5', 'w')
    train_num = 0
    for i in range(len(noise_list)):
        #n_img = cv2.imread(noise_list[i])
        #gt_img = cv2.imread(gt_list[i])

        n_img = Image.open(os.path.join(root,'DIV2K_train_LR', noise_list[i]))
        gt_img = Image.open(os.path.join(root,'DIV2K_train_HR', gt_list[i]))
...
class Dataset(udata.Dataset):
    def __init__(self, train=True):
        ...
        if self.train:
            h5f = h5py.File('data/train16.h5', 'r')
        else:
